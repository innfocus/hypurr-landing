...
export async function calculateMaxSwap(
	provider: BrowserProvider,
	signer: JsonRpcSigner,
	amount: string
) {
	...
	const slippage =
		(expectedAmountOut * BigInt(Math.floor(slippageTolerance * 100))) / BigInt(10000)
	const amountOutMinParsed = expectedAmountOut - slippage
...
async function swapTokenForETH(
	...
	const slippageFactor = (BigInt(100 - slippageTolerance) * parseUnits('1', 18)) / 100n
...
export { swapETHForToken, swapTokenForETH }
